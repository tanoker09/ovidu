...
public class ContestantController {
	...
	private Logger log = LoggerFactory.getLogger(ContestantController.class);
	...
	public ContestsResponse getContests(HttpServletRequest request) {
		log.info("Contestant is getting all contests");
		UserModel user = userAuthenticator.getUserFromRequest(request);
		List<UserContestModel> userContests = membershipService.findUserContestsWithStatus(user, "active");
	...
	public TokenResponse getOpenViduToken(HttpServletRequest request, @PathVariable("guid") String contestGuid) throws Exception {
		log.info("Contestant is getting contest {} streaming token", contestGuid);
		UserModel user = userAuthenticator.getUserFromRequest(request);
